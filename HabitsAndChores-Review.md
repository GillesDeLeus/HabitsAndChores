...
<p class="meta">Updated 18 June 2026 · iOS 17+ · SwiftUI + SwiftData + CloudKit · ~9,000 lines Swift across 76 files (incl. 128 unit tests) · public repo <code>github.com/GillesDeLeus/HabitsAndChores</code> (MIT)</p>
...
- **Tested core.** A 128-case unit-test target covers the pure engines (scheduling, streaks, gamification — including shared household-chore contribution, friend graph, household occurrence/recurrence, shared-draft / to-do mapping including the persisted recurrence `anchorDate`, chore **rotation**, multi-assignee per-person check-off, the conflict-merge map semantics, and the offline-outbox serialization), the models, helpers, and the `FriendsModel` view-model, plus regression tests that keep the data model CloudKit-valid and the String Catalog fully translated.
...
<strong>Rev. 10 (engine hardening).</strong> <span class="tag done">DONE</span> Streak/summary recompute is now linear: a per-task `CompletionIndex` (start-of-day `Set`s) replaces the per-occurrence linear scans of `isCompleted(on:)`, removing the O(occurrences × completions) cost on the completion-tap path; the summary builds each task's index once and reuses it for streaks, the weekly goal, and the perfect-week scan. <span class="tag done">DONE</span> `.everyN` monthly now clamps the anchor day to short months (consistent with `.monthly`) and counts whole months by month-index, fixing tasks anchored on the 29th–31st silently skipping February. <span class="tag done">DONE</span> `TodayView`'s "today" refreshes on `NSCalendarDayChanged` + foreground (no longer frozen at view-init across midnight). <span class="tag done">DONE</span> `.everyN` reminders (a finite rolling window of one-shot triggers) are topped up on launch/foreground so they no longer silently stop; reminder rescheduling now awaits cancellation before re-adding to avoid a duplicate/None race. New regression tests cover the everyN-month clamp and skip-neutral streak behaviour (66 tests total).

<strong>Rev. 11 (flexible recurrence).</strong> <span class="tag done">DONE</span> New <code>FrequencyRule.floating</code> kind — "once a week / once a month" with **no fixed day**. Unlike day-scheduled rules, a floating task maps *every* day of its period to one canonical occurrence (the period start) via the new <code>SchedulingEngine.occurrenceDate(for:on:)</code>, where its single per-period completion is keyed. It stays outstanding in **Today** (and the widget) every day of the week/month until completed once, then drops off; <code>isScheduled</code> stays true only on the period-start day so the calendar/stats still show one occurrence per period, and streaks count whole periods (the in-progress period never breaks a run). Floating is **private-only** for now: the editor's Household picker coerces it to weekly/monthly, so shared floating chores remain a follow-up. New regression tests cover floating occurrences, the constant period key, the start-date guard, and floating streaks (73 tests total).

<strong>Rev. 12 (household gamification, stable anchor, load gate).</strong> <span class="tag done">DONE</span> A shared **recurring chore** you complete now feeds the same gamification as a personal task — points, level, badges, streaks, and the weekly goal — via <code>HouseholdsModel.mySharedChoreStats()</code> → <code>GamificationEngine.summary(for:shared:)</code> (completions retained on load as <code>SharedCompletionFact</code> and matched to the current user by resolved member name; credit lags one household refresh). Shared **to-dos** still don't gamify (mirroring personal to-dos); the Stats tab and widget stay personal-only. <span class="tag done">DONE</span> The recurrence **anchor** is now a persisted, immutable <code>SharedChore.anchorDate</code> (captured client-side at creation, preserved across edits) instead of the CloudKit <code>creationDate</code> — fixing chores that vanished from past calendar days when an offline-created chore was replayed (the server re-stamped its creation time, shifting every occurrence forward). Legacy chores fall back to <code>creationDate</code>. <span class="tag done">DONE</span> A first-load gate (<code>HouseholdsModel.hasLoadedHouseholds</code>) makes **Today** wait for the initial CloudKit fetch and show a spinner, so it no longer reveals personal tasks first and pops household tasks in seconds later; the Tasks / To-Do lists gate only their empty state. New regression tests cover shared-chore gamification and the persisted anchor (119 tests total). <em>Schema note:</em> the new <code>SharedChore.anchorDate</code> field needs the Production CloudKit deploy.

<strong>Rev. 13 (multi-assignee chores — per-person check-off).</strong> <span class="tag done">DONE</span> A shared task can now be assigned to <strong>several members at once</strong>, each with their own checkbox: the chore is fully done only once <em>every</em> assignee has completed the current occurrence ("2 of 3 done" along the way), and each person's own completion still feeds their gamification. The single <code>SharedChore.assignee: String?</code> became an <code>assignees: [String]</code> list (with a new CloudKit <code>assignees</code> field; legacy records fall back to the old single <code>assignee</code>), and per-occurrence done-state is now derived from the set of <code>SharedCompletion.completedBy</code> for that occurrence (<code>completedByMembers</code>), with <code>isDone</code> baked to the current user's perspective at load. Un-checking now removes <em>only your own</em> completion (it previously deleted every member's tick for the occurrence). The task and to-do editors swap the single Assignee picker for a multi-select member list; <strong>rotation</strong> stays a single-owner concept and is mutually exclusive with multiple assignees. Both editors, all the list rows (Today / Tasks / To-Do / Calendar / household detail) show the assignee list and per-person progress. New regression tests cover per-person done/full-done, multi-assignee Today visibility, and the per-person un-complete filter (124 tests total). <em>Schema note:</em> the new <code>SharedChore.assignees</code> list field needs the Production CloudKit deploy (not queried, so no index).

<strong>Rev. 14 (cross-user avatars + conflict-safe writes).</strong> <span class="tag done">DONE</span> <strong>Member avatars across the share boundary.</strong> Members now self-publish their small character <code>AvatarConfig</code> (the same one as their public profile) onto the household root record's new <code>memberAvatars</code> map — the avatar counterpart to <code>memberNames</code> (CloudKit won't hand you other users' avatars directly). Every member resolves and renders each other's face in the Members list and the multi-select assignee picker; members without a social account gracefully fall back to initials. Invites seed the invitee's name <em>and</em> avatar so they appear before they even open the app. (Full photos aren't published — only the tiny config — to keep the shared root record small; photo-in-household is a follow-up.) <span class="tag done">DONE</span> <strong>Conflict-safe writes.</strong> All household fetch-modify-save paths (chore edits, assignment, the member name/avatar maps, leave-time unassignment, and the offline-outbox chore upsert) now go through a <code>saveMerging</code> helper that, on CloudKit's <code>serverRecordChanged</code>, adopts the server's record (its change tag + any field another member changed concurrently) and <strong>re-applies the mutation on top</strong> before retrying — field-level last-write-wins instead of a hard failure or a whole-record clobber; the <code>memberNames</code>/<code>memberAvatars</code> maps re-merge per key, so two members publishing at once keep both entries. (Per-person completion records were already conflict-free, being additive.) New regression tests cover the pure map-merge semantics (128 tests total). <em>Schema note:</em> the new <code>Household.memberAvatars</code> field needs the Production CloudKit deploy (not queried, so no index).
...
| <span class="tag done">DONE</span> Summary key edge case | The Today cache key now counts done *and* skipped completions separately (and the active day), so a status-only flip recomputes. | — |
...
- **Household chores now gamify** — a shared *recurring chore* you complete contributes to points, level, badges, streaks and the weekly goal, just like a personal task (`HouseholdsModel.mySharedChoreStats()` → `GamificationEngine.summary(for:shared:)`). Credit lags by one household refresh (the completion record must round-trip CloudKit before it counts) and is matched to "you" by your resolved member name. Shared *to-dos* still don't gamify (mirroring personal to-dos), and the **Stats tab** and the **widget** remain personal-only.
...
| Tests | <span class="tag ok">Now present</span> — 128 unit tests across the pure engines, models, helpers, the `FriendsModel` view-model, household occurrence logic, shared-draft / to-do mapping (incl. the recurrence `anchorDate`), chore rotation, multi-assignee per-person check-off, the conflict-merge map semantics, shared-chore gamification, and outbox serialization, plus CloudKit-schema and String-Catalog-coverage regression tests. SwiftUI views and CloudKit I/O remain untested. |
...
- **Deploy the CloudKit schema Development → Production** — record types `Profile`, `Handle`, `FriendEdge` (with queryable `owner`/`other` indexes), `Report`, `HouseholdInvite` (with a queryable `invitee` index for in-app invitations), and the household types `Household`, `SharedChore`, `SharedCompletion`. **New in Rev. 8:** `SharedChore` gained fields `isTodo`, `dueDate`, `scheduledDate`, `priority`, `reminderMode`, `reminderDate`, `reminderOffset`, `reminderHour`, `reminderMinute`, `rotates`, and `Household` gained a `memberNames` field — these auto-create in Development on first write but must be deployed to Production before release. The app still runs against Development. **New since:** `SharedChore` also gained an `anchorDate` field (the immutable recurrence anchor — see §3; legacy records fall back to the CloudKit creation date) and an `assignees` string-list field (multi-assignee per-person check-off — see §3 Rev. 13; legacy records fall back to the old single `assignee` field, neither is queried), and `Household` gained a `memberAvatars` field (self-published member character avatars — see §3 Rev. 14; not queried).
...
- **CloudKit Production deploy** — the **#1 remaining blocker.** All record types/fields used by social + households (incl. `Report`, `HouseholdInvite`, `Household.memberNames`, `Household.memberAvatars`, and the `SharedChore` to-do/reminder/`rotates`/`scheduledDate`/`assignees` fields, with queryable indexes) must be deployed; a released build cannot use the Development environment.
...
- <span class="tag done">DONE</span> **Optional assignment rotation (Rev. 8)** — a chore can carry `rotates`; on completion the assignee advances to the next household member (round-robin, retreats on un-completion), or stays put when off. Rotation math is pure and unit-tested. Rotation is a single-owner concept and is mutually exclusive with multiple assignees (Rev. 13).
- <span class="tag done">DONE</span> **Multi-assignee chores — per-person check-off (Rev. 13)** — a shared task can be assigned to several members at once; each has their own checkbox and the chore is fully done only once every assignee completes the occurrence ("N of M done"). `SharedChore.assignee` became an `assignees: [String]` list (new CloudKit `assignees` field; legacy single-`assignee` records still load), per-occurrence done-state derives from the `completedByMembers` set, and un-checking removes only your own completion. Editors use a multi-select member list; rows show the assignee list + per-person progress. Pure logic unit-tested.
...
| <span class="tag low">LOW</span> Member identity | Mostly resolved (Rev. 8 names, **Rev. 14 avatars**): members self-publish their display name **and character avatar** to the household so others see both. Residual: a member invited by **link** who hasn't opened the app, or a non-social user, still falls back to "Owner/Member" + initials until they publish; full **photo** avatars aren't published in households (only the character config). |
| <span class="tag ok">RESOLVED (Rev. 14)</span> Conflict handling | Household fetch-modify-save now goes through `saveMerging`: on `serverRecordChanged` it re-applies the change onto the server record and retries (field-level last-write-wins; map fields re-merge per key). Per-person completions are additive and already conflict-free. |
...
- <span class="tag done">DONE</span> **Member avatars (Rev. 14)** — members self-publish their character avatar to the household (`memberAvatars` map), resolved and shown for every member in the Members list and the assignee picker; non-social members fall back to initials. Residual P2: full **photo** avatars in households, and mapping participants to their `SharedProfile` for a richer link.
- <span class="tag done">DONE</span> **Conflict-safe writes (Rev. 14)** — fetch-modify-save routes through `saveMerging`, which retries on `serverRecordChanged` by re-applying the mutation onto the server record (field-level last-write-wins; maps re-merge per key). Residual P3: member **role** management (remove/leave exist; promoting a co-owner does not) and owner-side cleanup of stale `HouseholdInvite` records.
<span class="tag done">DONE</span> **Fairness / completion-history UI (Rev. 8)** — `HouseholdHistoryView` shows each member's completion share over the last 30 days (bars) plus a recent-activity feed, built from `SharedCompletion` records. Shared *recurring-chore* completions now also feed **gamification** (points, badges, streaks, weekly goal — see §3); the **Stats tab** chart remains personal-only.
