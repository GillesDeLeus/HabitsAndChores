...
<p class="meta">Updated 18 June 2026 · iOS 17+ · SwiftUI + SwiftData + CloudKit · ~9,000 lines Swift across 76 files (incl. 124 unit tests) · public repo <code>github.com/GillesDeLeus/HabitsAndChores</code> (MIT)</p>
...
- **Tested core.** A 124-case unit-test target covers the pure engines (scheduling, streaks, gamification — including shared household-chore contribution, friend graph, household occurrence/recurrence, shared-draft / to-do mapping including the persisted recurrence `anchorDate`, chore **rotation**, multi-assignee per-person check-off, and the offline-outbox serialization), the models, helpers, and the `FriendsModel` view-model, plus regression tests that keep the data model CloudKit-valid and the String Catalog fully translated.
...
| Tests | <span class="tag ok">Now present</span> — 124 unit tests across the pure engines, models, helpers, the `FriendsModel` view-model, household occurrence logic, shared-draft / to-do mapping (incl. the recurrence `anchorDate`), chore rotation, multi-assignee per-person check-off, shared-chore gamification, and outbox serialization, plus CloudKit-schema and String-Catalog-coverage regression tests. SwiftUI views and CloudKit I/O remain untested. |
...
- **CloudKit Production deploy** — the **#1 remaining blocker.** All record types/fields used by social + households (incl. `Report`, `HouseholdInvite`, `Household.memberNames`, and the `SharedChore` to-do/reminder/`rotates`/`scheduledDate`/`assignees` fields, with queryable indexes) must be deployed; a released build cannot use the Development environment.
